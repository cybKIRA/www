...
class Orders extends Simpla
{
	...
	public function get_order($id)
	{
		...
		$query = $this->db->placehold("SELECT  o.id, o.delivery_id, o.delivery_price, o.separate_delivery,
										...
										o.date, o.user_id, o.name, o.address, o.phone, o.email, o.comment,o.time,o.change_rub, o.status,
	...
	*/	
	...
	public function add_purchase($purchase)
	{
		...
		if(!isset($purchase->price) && !empty($variant))
			$purchase->price = $variant->price*$purchase->amount;
			
		
		if(!isset($purchase->weight) && !empty($variant))
			$purchase->weight = $variant->weight*$purchase->amount;
