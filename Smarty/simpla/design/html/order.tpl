...
{capture name=tabs}
	{if in_array('orders', $manager->permissions)}
		...
		<li {if $order->status==3}class="active"{/if}><a href="index.php?module=OrdersAdmin&status=3">Завершены</a></li>
	{if $keyword}
...
<div id="name">
	...
	<select class=status name="status">
		...
		<option value='3' {if $order->status == 3}selected{/if}>Зевершен</option>
	</select>
	</h1>
...
<div id="order_details">
	...
	<ul class="order_details">
		...
		<li>
			...
			<div class="edit_order_detail" style='display:none;'>
			<textarea name="time">{$order->time|escape}</textarea>
			</div>
			<div class="view_order_detail">
				{$order->time|escape|nl2br}
			</div>
		</li>
		<li>
			<label class=property>Сдача с</label>
			<div class="edit_order_detail" style='display:none;'>
			<textarea name="change_rub">{$order->change_rub|escape}</textarea>
			</div>
			<div class="view_order_detail">
				{$order->change_rub|escape|nl2br}
			</div>
		</li>
	</ul>
	</div>
	...
	<div id="list" class="purchases">
		...
		<span class='vesa'>
			<div class="price cell">			
				<span class="view_purchase weight">{$purchase->weight}</span>
				<span class=edit_purchase style='display:none;'>
					<input type=text name=purchases[weight][{$purchase->id}] value='{$purchase->weight}' size=5>
				</span>
				{$purchase->variant->unit}				
			</div>
			
			<div class="price cell">
				<span class="view_purchase basePrice">{$purchase->price}</span>
				<span class=edit_purchase style='display:none;'>
		...
		<div id="new_purchase" class="row" style='display:none;'>
			...
			<div class="price cell">
				<input type=text name=purchases[weight][] value='' size=5>
			</div>
...
$(function() {
  ...
  function change_variant(element)
  {
	...
	$("[name*=weight]").change(function() {
		...
		//цена рядом
		var price = parseFloat($(this).closest(".vesa").find(".basePrice").html());
		//изначальный вес
		var baseWeight  =  parseFloat($(this).closest(".vesa").find(".weight").html());
		
		var newPrice = weight * price / baseWeight;
		
		$(this).closest(".vesa").find("[name*=price]").val(newPrice.toFixed(2));
