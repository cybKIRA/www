...
<div id="purchases">
	<table width=100%>
		...
		{assign var="sum" value="`$sum+$purchase->price`"}
		{/foreach}
		...
		<tr class="sum">
			...
			<th>
				 	{$sum|string_format:"%.2f"} {$currency->sign}
			</th>
		</tr>
...
<div id="total">
	<table>
		...
		<tr>
			<th>Итого</th>
			<td class="total"><span style="white-space: normal;">{$order->total_price|string_format:"%.2f"}</span>&nbsp;{$currency->sign}</td>
		</tr>
		...
		<tr>
			...
			{assign var="total" value="`$order->total_price + $order->delivery_price`"}
			<td class="total">{$total|string_format:"%.2f"} &nbsp;{$currency->sign}</td>
		</tr>
