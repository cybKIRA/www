...
{if $product->image}<img src="{$product->image->filename|resize:350:350}" alt="{$product->name}"/>{/if}
</div>
...
<div class="transition1 itemDetails">
	...
	*}
	...
	{* Название *}
	<h2 data-product="{$product->id}">{$product->name|escape|truncate:65:'...'}</h2>
	{if $product->city}
	...
	{if $product->variants|count > 0}
		...
		{elseif $product->variant->price > 0 && $product->variants|count > 1}
			...
			<div class="price-mini-block">
				{foreach $product->variants as $v name=variant}				
					<input type="radio" name="variant" value="{$v->id}" {if $smarty.foreach.variant.first}checked{/if}> {$v->name} <span class="price-mini"> {$v->price} {$currency->sign|escape} </span> <br>
				{/foreach}
			</div>
