	...
	{if $product->variants|count > 0}
		...
		{if $product->variant->price > 0 && $product->variants|count == 1}
			<div class="transition1 price">
				...
				<span style="display:none;" class="step_compare_price">{$product->variant->compare_price|convert}</span>
						...
						<div class="amount">
							<span style="display:none;" class="amount_step">{if $product->variant->unit != 'кг'}{$product->variant->weight|convert}{else}{$product->variant->weight}{/if}</span>
							<input style="display:none;" type="text" name="amount" value="1">
							<span class="clickable minus">-</span>
							<input class="text-center" type="text" name="weight" value="{if $product->variant->unit != 'кг'}{$product->variant->weight|convert}{else}{$product->variant->weight}{/if}">
		...
		{elseif $product->variant->price > 0 && $product->variants|count > 1}
			...
			<div class="price" title='Несколько вариантов предложения'>
 			...
 			<a class="btn-primary btn action to-cart" href="/products/{$product->url}"><i class="i-newspaper"></i>{$product->variants|count} {$product->variants|count|plural:'Вариант':'Вариантов':'Варианта'}</a>
