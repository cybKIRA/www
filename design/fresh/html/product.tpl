				...
				<form class="form variants" action="/cart">
					{if $product->variants|count > 0 && $product->variant->price >0}
						...
						<div class="price m-b-10">
							<span>{$product->variant->price|convert}</span>&nbsp;&nbsp;<i>{$currency->sign|escape}, {if $product->variant->unit} {$product->variant->weight} {$product->variant->unit}{else}{$settings->units}{/if}</i>
...
<div class="pwstabs">
	...
	<div class="itemFeatures" data-pws-tab="tab2" data-pws-tab-name="Характеристики" data-pws-tab-icon="i-doc-add">
			...
			{/foreach}
		<div class="clearfix"></div>
		<p class="text-xs p-15 text-transparent"></p>
	</div>
	{/if}
