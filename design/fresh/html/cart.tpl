...
<form method="post" name="cart">
	...
	<table class="table table-condensed table-hover cartTable text-left form ">
		...
		<div class="col-xs-12 col-md-7">
			<div class="h3 text-uppercase"><span>ВСПОСОБ ОПЛАТЫ</span> НАЛИЧНЫМИ КУРЬЕРУ</div>
		</div>
	...
	<div class="col-xs-12 col-md-7 form ">
		<div class="text-left deliveriesBlock">
			...
			{foreach $deliveries as $delivery}
				...
				{$delivery->name}
					<span class="color">{if $cart->total_price < $delivery->free_from && $delivery->price>0}	({$delivery->price|convert}&nbsp;{$currency->sign})
...
$(document).ready(function(){
   $("#phoneCart").inputmask("+9(999) 999-99-99");  //static mask
});
</script>
{/literal}