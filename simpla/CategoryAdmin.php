  ...
  function fetch()
  {
		...
		if($this->request->method('post'))
		{
			...
			else
			{
				...
				if($this->request->post('delete_thumb'))
  	    		{
  	    			$this->categories->delete_thumb($category->id);
  	    		}
				...
				$thumb = $this->request->files('thumb');
				
  	    		if(!empty($thumb['name']) && in_array(strtolower(pathinfo($thumb['name'], PATHINFO_EXTENSION)), $this->allowed_image_extentions))
  	    		{
  	    			$this->categories->delete_thumb($category->id);
  	    			move_uploaded_file($thumb['tmp_name'], $this->root_dir.$this->config->categories_thumbs_dir.$thumb['name']);
  	    			$this->categories->update_category($category->id, array('thumb'=>$thumb['name']));
  	    		}
