...
class Categories extends Simpla
{
	...
	public function delete_category($ids)
	{
		...
		foreach($ids as $id)
		{
			...
			$this->delete_thumb($category->children);
	...
	public function delete_thumb($categories_ids)
	{
		$categories_ids = (array) $categories_ids;
		$query = $this->db->placehold("SELECT thumb FROM __categories WHERE id in(?@)", $categories_ids);
		$this->db->query($query);
		$filenames = $this->db->results('thumb');
		if(!empty($filenames))
		{
			$query = $this->db->placehold("UPDATE __categories SET thumb=NULL WHERE id in(?@)", $categories_ids);
			$this->db->query($query);
			foreach($filenames as $filename)
			{
				$query = $this->db->placehold("SELECT count(*) as count FROM __categories WHERE thumb=?", $filename);
				$this->db->query($query);
				$count = $this->db->result('count');
				if($count == 0)
				{			
					@unlink($this->config->root_dir.$this->config->categories_thumbs_dir.$filename);		
				}
			}
			unset($this->categories_tree);
			unset($this->all_categories);	
	...
	private function init_categories()
	{
		...
		$query = $this->db->placehold("SELECT c.id, c.parent_id, c.name, c.description, c.url, c.meta_title, c.meta_keywords, c.meta_description, c.thumb, c.image, c.visible, c.position, COUNT(p.id) as products_count
