	...
	public function get_cart()
	{
		...
		if(!empty($_SESSION['shopping_cart']))
		{
			...
			if(!empty($variants))
			{
				...
				$cart->cart_price = $cart->total_price;
				...
				if(isset($_SESSION['coupon_code']))
				{
					...
					if($cart->coupon && $cart->coupon->valid && $cart->total_price>=$cart->coupon->min_order_price)
					{
						...
						{
							// GLOOBUS 2016-05-25 добавляем проверку купона на принадлежность к определенным товарам
							if ($cart->coupon->of_product) {
								// Бежим циклом по товарам, и если у товара встречается нужная нам скидка как у купона, то считаем ее
								foreach ($cart->purchases as $ps) {
									if ($cart->coupon->value <= $ps->product->sale) {
										$cart->coupon_discount += ($ps->variant->price * $ps->amount) * ($cart->coupon->value)/100;
									}										
								}								
