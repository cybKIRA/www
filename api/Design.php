...
class Design extends Simpla
{
	...
	public function resize_modifier($filename, $width=0, $height=0, $set_watermark=false)
	{
		...
		//McBronx 06.06.16 каструруем токены
		$size = $width.'x'.$height;
        $image_sizes = array();
		...
		return $this->config->root_url.'/'.$this->config->resized_images_dir.$resized_filename_encoded/*McBronx касртриуем токены .'?'.$this->config->token($resized_filename)*/;
	}
