...
class Products extends Simpla
{
	...
	*/
	...
	/**
		#Ник	
	* Расширенная функция возврата товара по ID,
	* дополнительно возвращает поля по варианту в едином массиве
	* @param	$id, Объект варианта
	* @retval	array object
	*/
	public function get_product_var($var) 
	{
		$prod = $this->products->get_product((int)$var->product_id);
			$prod_result = array (
				'id'=>$prod->id,
				'id_post'=>$var->id_post,
				'name'=>$prod->name,
	...
	*/	
	...
	public function increase_rate_product($id, $rate)
	{
		$product = $this->products->get_product($id);
		$this->products->update_product($id, array('votes' => intval($product->votes + 1), 'rating' => intval($product->rating + $rate)));
	}	
	
	public function decrease_rate_product($id, $rate)
	{
		$product = $this->products->get_product($id);
		$this->products->update_product($id, array('votes' => intval($product->votes - 1), 'rating' => intval($product->rating - $rate)));
	}
