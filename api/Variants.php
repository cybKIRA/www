...
class Variants extends Simpla
{
	...
	*/
	...
	{		
		...
		//если ID - пустой выведем все варианты, для этого ниже закоментировал #Ник
		//if(!$product_id_filter && !$variant_id_filter)
			//return array();
		
		//Расширение возможности отфильтровки по ид поставщика #Ник
		if(!empty($filter['id_post']))
			$post_id_filter = $this->db->placehold('AND v.id_post in(?@)', (array)$filter['id_post']);
		
		$query = $this->db->placehold("SELECT v.id, v.product_id, v.id_post , v.price, v.price_post, NULLIF(v.compare_price, 0) as compare_price, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.name, v.attachment, v.position
	...
	{	
		...
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.id_post, v.price, v.price_post, NULLIF(v.compare_price, 0) as compare_price, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.name, v.attachment
