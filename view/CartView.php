...
class CartView extends View
{
  ...
  public function __construct()
  {
    ...
    if(isset($_POST['checkout']))
    {
		...
		$order->email       = "client@rfresh.ru";
    	$order->address     = $this->request->post('address');
    	$order->phone       = $this->request->post('phone');
    	$order->comment     = $this->request->post('comment');
		//Mcbronx - добавим два поля
		$order->time     = $this->request->post('time');
		$order->change_rub   = $this->request->post('change_rub');
		...
		// GLOOBUS 2016-06-22 ошибку, если не введен телефон
	...
	{  
		...
		if($this->user)
		{
			...
			else
			{
				...
				// GLOOBUS 2016-06-22 еще берем телефон
